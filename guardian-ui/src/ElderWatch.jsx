...
export default function ElderWatch() {
  ...
  // Each elderlyId gets fully isolated state on the backend.
  useEffect(() => {
    post("/gps/config", {
  ...
  useEffect(() => {
    if (homeMarkerRef.current) homeMarkerRef.current.setLatLng([home.lat, home.lng]);
  ...
  // Poll real GPS position — uses ?elderlyId so only this account's data is fetched
  useEffect(() => {
    ...
    async function pollPosition() {
      ...
      const newLat = data.lat, newLng = data.lng;
      ...
      setStatusText(prev => {
        if (prev === "Home" && !isHome) {
          setAlerts(a => [{ id: Date.now(), type: "left", time: new Date().toISOString(), distance: currentDist }, ...a].slice(0, 20));
          showToast("left", "Left Home Zone", `${currentDist}m from home`);
        } else if (prev === "Outside" && isHome) {
          setAlerts(a => [{ id: Date.now(), type: "entered", time: new Date().toISOString(), distance: currentDist }, ...a].slice(0, 20));
    ...
    <div className="ew-app">
      ...
      <div className="ew-header">
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span className={`ew-badge ${isHome ? "ew-badge--green" : "ew-badge--red"}`}>{isHome ? "HOME" : "OUTSIDE"}</span>
          <span style={{ fontSize: "0.75rem", fontFamily: "var(--font-mono)", color: "var(--green)" }}>LIVE</span>
        </div>
      </div>
      ...
      <div className="ew-main">
        ...
        <div className="ew-right">
          ...
          <div className="ew-card" style={{ borderColor: isHome ? "rgba(45,122,80,.3)" : "rgba(212,90,90,.3)" }}>
            ...
            <div style={{ textAlign: "center", fontSize: "2.5rem", margin: "8px 0" }}>{isHome ? "\u{1f3e0}" : "\u{1f6b6}"}</div>
          ...
          <div className="ew-card">
            ...
            {TRACKING_MODES.map(m => (
              <div key={m.id} className={`ew-mode-btn ${mode === m.id ? "ew-mode-btn--active" : ""}`} onClick={() => setMode(m.id)}>
                <div><div className="ew-mode-name">{m.name}</div><div className="ew-mode-sub">{m.desc}</div></div>
              </div>
            ))}
          </div>
